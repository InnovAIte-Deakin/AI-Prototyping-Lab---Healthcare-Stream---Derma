...
describe('Doctor flows', () => {
  ...
  it('fetches patient reports and displays risk/advice on detail page', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('report-5-risk')).toHaveTextContent('Risk: High');
      expect(screen.getByTestId('report-5-advice')).toHaveTextContent('Advice: Schedule an in-person consult');
    });
